...
# setwd("/path/to/csmGmm_reproduce/SuppFig17/SuppFig17B_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig17/SuppFig17B_sim.R")
...
outName <- paste0(outputDir, "/SFig17B_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig17B_allZ")
betaName <- here::here(outputDir, "SFig17B_allBeta")
...
betaOpts <- cbind(c(0.2 + 0.01 * effSizeMult, 0.3 + 0.01 * effSizeMult),
                                    c(0.2 + 0.01 * effSizeMult, 0.3 + 0.01 * effSizeMult))
betaMin <- betaOpts[1, ]
betaMax <- betaOpts[2, ]
beta0 <- -1
