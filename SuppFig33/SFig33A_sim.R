...
# setwd("/path/to/csmGmm_reproduce/SuppFig33/SFig33A_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig33/SFig33A_sim.R")
...
outName <- paste0(outputDir, "/SFig33A_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig33A_allZ")
betaName <- here::here(outputDir, "SFig33A_allBeta")
...
mu1 <- 1 + 0.1 * effSizeMult
mu2 <- 2 + 0.1 * effSizeMult
mu3 <- 3 + 0.1 * effSizeMult
...
genData <- function(hMat, nSNPs, corMat, mu1, mu2, mu3) {
  ...
  n3 <- sum(hMat$number[which(abs(hMat$Var1) == 1 & abs(hMat$Var2) == 1)]) / 4
  numSigs <- 2*n1 + 2*n2 + 4*n3
  sigLocs <- sample(x=1:nSNPs, size=numSigs, replace=FALSE)
  sigMat <- matrix(data=0, nrow=nSNPs, ncol=2)
  sigMat[sigLocs[1:n1], 2] <- mu1
  sigMat[sigLocs[(n1+1):(2*n1)], 2] <- -mu1
  sigMat[sigLocs[(2*n1+1):(2*n1+n2)], 1] <- mu2
  sigMat[sigLocs[(2*n1+n2+1):(2*n1+2*n2)], 1] <- -mu2
  sigMat[sigLocs[(2*n1+2*n2+1):(2*n1+2*n1+n3)], 1:2] <- mu3
  sigMat[sigLocs[(2*n1+2*n1+n3+1):(2*n1+2*n1+2*n3)], 1] <- mu3
  sigMat[sigLocs[(2*n1+2*n1+n3+1):(2*n1+2*n1+2*n3)], 2] <- -mu3
  sigMat[sigLocs[(2*n1+2*n1+2*n3+1):(2*n1+2*n1+3*n3)], 1] <- -mu3
  sigMat[sigLocs[(2*n1+2*n1+2*n3+1):(2*n1+2*n1+3*n3)], 2] <- mu3
  sigMat[sigLocs[(2*n1+2*n1+3*n3+1):(2*n1+2*n1+4*n3)], 1:2] <- -mu3
...
# record results here
powerRes <- data.frame(nCausal=rep(NA, nSims),  minEff1=mu1,
