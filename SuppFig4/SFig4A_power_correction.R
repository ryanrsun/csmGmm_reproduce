...
outputDir <- "/rsrch3/home/biostatistics/rsun3/empBayes/reproduce/SuppFig4/output"
outName <- paste0("Power_correctionS4A_S", Snum, "_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- paste0("Power_correctionS4A_S", Snum, "_allZ")
betaName <- paste0("Power_correctionS4A_S", Snum, "_allBeta")
#-------------------------------------------------------------------#
...
betaMin <- c(0.18 + 0.01 * effSizeMult, 0.18 + 0.01 * effSizeMult, 0.18 + 0.01 * effSizeMult)
