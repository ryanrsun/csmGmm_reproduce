...
# setwd("/path/to/csmGmm_reproduce/SuppFig14/SuppFig14B1_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig14/SuppFig14B1_sim.R")
...
outName <- paste0(outputDir, "/SFig14B1_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig14B1_allZ")
betaName <- here::here(outputDir, "SFig14B1_allBeta")
...
betaMin <- c(0.35, 0.35, 0.35)
betaMax <- c(0.35, 0.35, 0.35)
beta0 <- -1
