...
# setwd("/path/to/csmGmm_reproduce/SuppFig14/SuppFig14A1_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig14/SuppFig14A1_sim.R")
...
outName <- paste0(outputDir, "/SFig14A1_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig14A1_allZ")
betaName <- here::here(outputDir, "SFig14A1_allBeta")
...
nSets <- nSNPs / setSize
nSims <- 2
margprob <- rep(0.1, setSize)
simsPerEffSize <- 50
effSizeMult <- ceiling(aID / simsPerEffSize)
betaMin <- c(0.3 + 0.01 * effSizeMult, 0.3 + 0.01 * effSizeMult, 0.3 + 0.01 * effSizeMult)
betaMax <- c(0.4 + 0.01 * effSizeMult, 0.4 + 0.01 * effSizeMult, 0.4 + 0.01 * effSizeMult)
beta0 <- -1
