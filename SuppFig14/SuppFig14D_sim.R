...
# setwd("/path/to/csmGmm_reproduce/SuppFig14/SuppFig14D_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig14/SuppFig14D_sim.R")
...
outName <- paste0(outputDir, "/SFig14D_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig14D_allZ")
betaName <- here::here(outputDir, "SFig14D_allBeta")
...
betaOpts <- cbind(c(1.6 + 0.01 * effSizeMult, 1.7 + 0.01 * effSizeMult),
                                    c(1.6 + 0.01 * effSizeMult, 1.7 + 0.01 * effSizeMult))
betaMin <- betaOpts[1, ]
betaMax <- betaOpts[2, ]
beta0 <- -1
