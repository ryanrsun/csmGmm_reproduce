...
# setwd("/path/to/csmGmm_reproduce/SuppFig24/SFig24A_sim_fit3.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig24/SFig24A_sim_fit3.R")
...
outName <- paste0(outputDir, "/SFig24A_aID", aID, "_fit3.txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig24Af3_allZ")
betaName <- here::here(outputDir, "SFig24Af3_allBeta")
...
for (sim_it in 1:nSims) {
  ...
  # new method
  if (doNew) {
    initPiList <- list(c(0.82), c(0.04/3, 0.04/3, 0.04/3), c(0.04/3, 0.04/3, 0.04/3), c(0.1/3, 0.1/3, 0.1/3))
