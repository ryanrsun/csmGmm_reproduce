...
# setwd("/path/to/csmGmm_reproduce/SuppFig24/SFig24A_sim_fit4.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig24/SFig24A_sim_fit4.R")
...
outName <- paste0(outputDir, "/SFig24A_aID", aID, "_fit4.txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig24Af4_allZ")
betaName <- here::here(outputDir, "SFig24Af4_allBeta")
...
betaOptsNon <- cbind(c(0 + effSizeMult * 0.01, 0.03 + effSizeMult * 0.01, 0.07 + effSizeMult * 0.01),
                     c(0 + effSizeMult * 0.01, 0.03 + effSizeMult * 0.01, 0.07 + effSizeMult * 0.01))
betaOptsCausal <- cbind(c(0.07 + effSizeMult * 0.01, 0.1 + effSizeMult * 0.01, 0.15 + effSizeMult * 0.01),
                        c(0.07 + effSizeMult * 0.01, 0.1 + effSizeMult * 0.01, 0.15 + effSizeMult * 0.01))
beta0 <- -1

# allocate snps new function for set mbl
allocate_sigs_mbl <- function(nSNPs) {

  # 5% pct in (1,0) and 5% in (0, 1) and 0.75% in alt
  totSigs <- 4116
  ...
  sigLocsMat[1:(6 * pctIt), 1] <- tempPos[1:(6 * pctIt)]
  sigLocsMat[1:pctIt, 3] <- 1
  sigLocsMat[(1 * pctIt + 1):(2 * pctIt), 3] <- 2
  sigLocsMat[(2 * pctIt + 1):(3 * pctIt), 3] <- 3
  sigLocsMat[(3 * pctIt + 1):(4 * pctIt), 3] <- -1
  sigLocsMat[(4 * pctIt + 1):(5 * pctIt), 3] <- -2
  sigLocsMat[(5 * pctIt + 1):(6 * pctIt), 3] <- -3
  # now the (0, 1) and (0, -1)
  sigLocsMat[(6 * pctIt + 1):(12 * pctIt), 2] <- tempPos[(6 * pctIt + 1):(12 * pctIt)]
  sigLocsMat[(6 * pctIt + 1):(7 * pctIt), 4] <- 1
  sigLocsMat[(7 * pctIt + 1):(8 * pctIt), 4] <- 2
  sigLocsMat[(8 * pctIt + 1):(9 * pctIt), 4] <- 3
  sigLocsMat[(9 * pctIt + 1):(10 * pctIt), 4] <- -1
  sigLocsMat[(10 * pctIt + 1):(11 * pctIt), 4] <- -2
  sigLocsMat[(11 * pctIt + 1):(12 * pctIt), 4] <- -3
  ...
  sigLocsMat[(tempStart + 3 * altIt + 1):(tempStart + 4 * altIt), 3] <- 1
  sigLocsMat[(tempStart + 3 * altIt + 1):(tempStart + 4 * altIt), 4] <- -1
  sigLocsMat[(tempStart + 4 * altIt + 1):(tempStart + 5 * altIt), 3] <- 2
  sigLocsMat[(tempStart + 4 * altIt + 1):(tempStart + 5 * altIt), 4] <- -2
  sigLocsMat[(tempStart + 5 * altIt + 1):(tempStart + 6 * altIt), 3] <- 3
  sigLocsMat[(tempStart + 5 * altIt + 1):(tempStart + 6 * altIt), 4] <- -3
  # (now the (-1, 1) and (-1, -1)
  sigLocsMat[(tempStart + 6 * altIt + 1):(tempStart + 7 * altIt), 3:4] <- -1
  sigLocsMat[(tempStart + 7 * altIt + 1):(tempStart + 8 * altIt), 3:4] <- -2
  sigLocsMat[(tempStart + 8 * altIt + 1):(tempStart + 9 * altIt), 3:4] <- -3
  sigLocsMat[(tempStart + 9 * altIt + 1):(tempStart + 10 * altIt), 3] <- -1
  sigLocsMat[(tempStart + 9 * altIt + 1):(tempStart + 10 * altIt), 4] <- 1
  sigLocsMat[(tempStart + 10 * altIt + 1):(tempStart + 11 * altIt), 3] <- -2
  sigLocsMat[(tempStart + 10 * altIt + 1):(tempStart + 11 * altIt), 4] <- 2
  sigLocsMat[(tempStart + 11 * altIt + 1):(tempStart + 12 * altIt), 3] <- -3
  sigLocsMat[(tempStart + 11 * altIt + 1):(tempStart + 12 * altIt), 4] <- 3
...
for (sim_it in 1:nSims) {
  ...
  # new method
  if (doNew) {
    initPiList <- list(c(0.2), c(0.2/4, 0.2/4, 0.2/4, 0.2/4), c(0.2/4, 0.2/4, 0.2/4, 0.2/4), c(0.4/4, 0.4/4, 0.4/4, 0.4/4))
    initMuList <- list(matrix(data=0, nrow=2, ncol=1), matrix(data=c(0, 2, 0, 3, 0, 5, 0, 6), nrow=2),
                     matrix(data=c(2, 0, 3, 0, 5, 0, 6, 0), nrow=2), matrix(data=c(7, 7, 8, 8, 9, 9, 10, 10), nrow=2))
