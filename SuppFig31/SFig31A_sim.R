...
# setwd("/path/to/csmGmm_reproduce/SuppFig31/SFig31A_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig31/SFig31A_sim.R")
...
outputDir <- here::here("SuppFig31", "output")
outName <- paste0(outputDir, "/SFig31A_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig31A_allZ")
betaName <- here::here(outputDir, "SFig31A_allBeta")
...
betaOpts <- cbind(c(0.00 + 0.01 * effSizeMult, 0.1 + 0.01 * effSizeMult),
                  c(0.04 + 0.01 * effSizeMult, 0.14 + 0.01 * effSizeMult))
betaMin <- betaOpts[1, ]
betaMax <- betaOpts[2, ]
beta0 <- -1
...
for (sim_it in 1:nSims) {
  ...
  # maxP method
  maxpDat <- data.frame(origIdx = 1:nrow(allP), maxP=apply(allP[, 1:2], 1, max)) %>%
    arrange(maxP) %>%
    mutate(rankedIdxP = 1:nrow(.)) %>%
    mutate(km = 1:nrow(.) / nrow(.)) %>%
    mutate(RHS = km * 0.1)
  maxpReject <- which(maxpDat$maxP <= maxpDat$RHS)
  if (length(maxpReject) == 0) {
    maxpMaxIdx <- 0
  } else { maxpMaxIdx <- max(maxpReject) }
  maxpDat <- maxpDat %>% mutate(maxpReject = ifelse(rankedIdxP <= maxpMaxIdx, 1, 0)) %>%
    arrange(origIdx)
  # append
  totOut <- totOut %>% mutate(maxP = maxpDat$maxP, maxpReject = maxpDat$maxpReject)
  powerRes$nRejMaxp[sim_it] <- length(which(totOut$maxpReject == 1))
  powerRes$fdpMaxp[sim_it] <- length(which(totOut$maxpReject == 1 & totOut$causal == 0)) / length(which(totOut$maxpReject == 1))
  powerRes$powerMaxp[sim_it] <- length(which(totOut$maxpReject == 1 & totOut$causal == 1))  / sum(causalVec)
