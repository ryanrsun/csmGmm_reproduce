...
# setwd("/path/to/csmGmm_reproduce/SuppFig19/SuppFig19A_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig19/SuppFig19A_sim.R")
...
outName <- paste0(outputDir, "/SFig19A_aID", Snum, "_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig19A_allZ")
betaName <- here::here(outputDir, "SFig19A_allBeta")
...
betaOpts <- cbind(c(0.38, 0.38), c(0.38, 0.38))
betaMin <- betaOpts[1, ]
betaMax <- betaOpts[2, ]
beta0 <- -1
...
for (row_it in 1:99) {
  for (col_it in row_it:99) {
    ...
    maxCor <- min(sqrt(mp1 * (1 - mp2) / (mp2 * (1 - mp1))), sqrt(mp2 * (1 - mp1) / (mp1 * (1 - mp2))))
    tempCor <- min(maxCor, outcomeCor)
    ...
    found <- FALSE
    while(!found & tempCor > 0) {
      tempBC <- matrix(data=c(1, tempCor, tempCor, 1), nrow=2)
      tempCP <- bincorr2commonprob(margprob=c(mp1, mp2), bincorr = tempBC)
      tempSig <- tryCatch(commonprob2sigma(commonprob = tempCP), error=function(e) e)
      if (class(tempSig)[1] == "simpleError") {
        tempCor <- tempCor - 0.01
      }  else {
        fillVal <- tempSig[1, 2]
        found <- TRUE
      }
    }
    sigmaValsMat[row_it, col_it] <- fillVal
    sigmaValsMat[col_it, row_it] <- fillVal
  }
}
