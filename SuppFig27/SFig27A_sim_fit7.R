...
outName <- paste0(outputDir, "/SFig27A_aID", aID, "_fit7.txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig27Af7_allZ")
betaName <- here::here(outputDir, "SFig27Af7_allBeta")
...
for (sim_it in 1:nSims) {
  ...
  # new method
  if (doNew) {
    initPiList <- list(c(0.2), c(0.2/7, 0.2/7, 0.2/7, 0.2/7, 0.2/7, 0.2/7), c(0.2/7, 0.2/7, 0.2/7, 0.2/7, 0.2/7, 0.2/7, 0.2/7), c(0.4/7, 0.4/7, 0.4/7, 0.4/7, 0.4/7, 0.4/7, 0.4/7))
    initMuList <- list(matrix(data=0, nrow=2, ncol=1), matrix(data=c(0, 1, 0, 1.5, 0, 2, 0, 2.5, 0, 3, 0, 3.5, 0, 4), nrow=2),
                     matrix(data=c(1, 0, 1.5, 0, 2, 0, 2.5, 0, 3, 0, 3.5, 0, 4, 0), nrow=2), matrix(data=c(4, 4, 4.5, 4.5, 5, 5, 5.5, 5.5, 6, 6, 7, 7, 8, 8), nrow=2))
