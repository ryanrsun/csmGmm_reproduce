...
# setwd("/path/to/csmGmm_reproduce/SuppFig27/SFig27A_sim_fit1.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig27/SuppFig27/SFig27A_sim_fit1.R")
...
outName <- paste0(outputDir, "/SFig27A_aID", aID, "_fit1.txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig27Af1_allZ")
betaName <- here::here(outputDir, "SFig27Af1_allBeta")
...
for (sim_it in 1:nSims) {
  ...
  if (doNew) {
    ...
    initMuList <- list(matrix(data=0, nrow=2, ncol=1), matrix(data=c(0, 1), nrow=2),
                     matrix(data=c(1, 0), nrow=2), matrix(data=c(3, 3), nrow=2)) 
