...
# setwd("/path/to/csmGmm_reproduce/SuppFig25/SFig25A_sim_fit4.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig25/SFig25A_sim_fit4.R")
...
outName <- paste0(outputDir, "/SFig25A_aID", aID, "_fit4.txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig25Af4_allZ")
betaName <- here::here(outputDir, "SFig25Af4_allBeta")
...
allocate_sigs_mbl <- function(nSNPs) {
  ...
  sigLocsMat[1:(6 * threePct), 1] <- tempPos[1:(6 * threePct)]
  sigLocsMat[1:threePct, 3] <- 1
  sigLocsMat[(1 * threePct + 1):(2 * threePct), 3] <- 2
  sigLocsMat[(2 * threePct + 1):(3 * threePct), 3] <- 3
  sigLocsMat[(3 * threePct + 1):(4 * threePct), 3] <- -1
  sigLocsMat[(4 * threePct + 1):(5 * threePct), 3] <- -2
  sigLocsMat[(5 * threePct + 1):(6 * threePct), 3] <- -3
  # now the (0, 1) and (0, -1)
  sigLocsMat[(6 * threePct + 1):(12 * threePct), 2] <- tempPos[(6 * threePct + 1):(12 * threePct)]
  sigLocsMat[(6 * threePct + 1):(7 * threePct), 4] <- 1
  sigLocsMat[(7 * threePct + 1):(8 * threePct), 4] <- 2
  sigLocsMat[(8 * threePct + 1):(9 * threePct), 4] <- 3
  sigLocsMat[(9 * threePct + 1):(10 * threePct), 4] <- -1
  sigLocsMat[(10 * threePct + 1):(11 * threePct), 4] <- -2
  sigLocsMat[(11 * threePct + 1):(12 * threePct), 4] <- -3

  # now the (1, 1) and (1, -1)
  sigLocsMat[(12 * threePct + 1):(18 * threePct), 1:2] <- tempPos[(12 * threePct + 1):(18 * threePct)]
  sigLocsMat[(12 * threePct + 1):(13 * threePct), 3:4] <- 1
  sigLocsMat[(13 * threePct + 1):(14 * threePct), 3:4] <- 2
  sigLocsMat[(14 * threePct + 1):(15 * threePct), 3:4] <- 3
  sigLocsMat[(15 * threePct + 1):(16 * threePct), 3] <- 1
  sigLocsMat[(15 * threePct + 1):(16 * threePct), 4] <- -1
  sigLocsMat[(16 * threePct + 1):(17 * threePct), 3] <- 2
  sigLocsMat[(16 * threePct + 1):(17 * threePct), 4] <- -2
  sigLocsMat[(17 * threePct + 1):(18 * threePct), 3] <- 3
  sigLocsMat[(17 * threePct + 1):(18 * threePct), 4] <- -3
  # (now the (-1, 1) and (-1, -1)
  sigLocsMat[(18 * threePct + 1):(24 * threePct), 1:2] <- tempPos[(18 * threePct + 1):(24 * threePct)]
  sigLocsMat[(18 * threePct + 1):(19 * threePct), 3:4] <- -1
  sigLocsMat[(19 * threePct + 1):(20 * threePct), 3:4] <- -2
  sigLocsMat[(20 * threePct + 1):(21 * threePct), 3:4] <- -3
  sigLocsMat[(21 * threePct + 1):(22 * threePct), 3] <- -1
  sigLocsMat[(21 * threePct + 1):(22 * threePct), 4] <- 1
  sigLocsMat[(22 * threePct + 1):(23 * threePct), 3] <- -2
  sigLocsMat[(22 * threePct + 1):(23 * threePct), 4] <- 2
  sigLocsMat[(23 * threePct + 1):(24 * threePct), 3] <- -3
  sigLocsMat[(23 * threePct + 1):(24 * threePct), 4] <- 3
...
for (sim_it in 1:nSims) {
  ...
  if (doNew) {
    ...
    initMuList <- list(matrix(data=0, nrow=2, ncol=1), matrix(data=c(0, 1, 0, 2, 0, 3, 0, 4), nrow=2),
                     matrix(data=c(1, 0, 2, 0, 3, 0, 4, 0), nrow=2), matrix(data=c(4, 4, 5, 5, 6, 6, 7, 7), nrow=2)) 
