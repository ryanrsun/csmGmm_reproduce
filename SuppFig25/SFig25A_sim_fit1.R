...
# setwd("/path/to/csmGmm_reproduce/SuppFig25/SFig25A_sim_fit1.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig25/SFig25A_sim_fit1.R")
...
outName <- paste0(outputDir, "/SFig25A_aID", aID, "_fit1.txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig25Af1_allZ")
betaName <- here::here(outputDir, "SFig25Af1_allBeta")
...
for (sim_it in 1:nSims) {
  ...
  # new method
  if (doNew) {
    initPiList <- list(c(0.2), c(0.2), c(0.2), c(0.4))
    initMuList <- list(matrix(data=0, nrow=2, ncol=1), matrix(data=c(0, 2), nrow=2),
                     matrix(data=c(2, 0), nrow=2), matrix(data=c(4, 4), nrow=2))
