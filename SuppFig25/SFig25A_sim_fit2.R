...
# setwd("/path/to/csmGmm_reproduce/SuppFig25/SFig25A_sim_fit2.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig25/SFig25A_sim_fit2.R")
...
outName <- paste0(outputDir, "/SFig25A_aID", aID, "_fit2.txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig25Af2_allZ")
betaName <- here::here(outputDir, "SFig25Af2_allBeta")
...
for (sim_it in 1:nSims) {
  ...
  # new method
  if (doNew) {
    initPiList <- list(c(0.2), c(0.1, 0.1), c(0.1, 0.1), c(0.2, 0.2))
    initMuList <- list(matrix(data=0, nrow=2, ncol=1), matrix(data=c(0, 2, 0, 3), nrow=2),
                     matrix(data=c(2, 0, 3, 0), nrow=2), matrix(data=c(4, 4, 5, 5), nrow=2)) 
