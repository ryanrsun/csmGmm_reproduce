...
# setwd("/path/to/csmGmm_reproduce/SuppFig32/SFig32A_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig32/SFig32A_sim.R")
...
outName <- paste0(outputDir, "/SFig32A_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig32A_allZ")
betaName <- here::here(outputDir, "SFig32A_allBeta")
...
mu1 <- 3 + 0.1 * effSizeMult
mu2 <- 3 + 0.1 * effSizeMult
