...
# setwd("/path/to/csmGmm_reproduce/SuppTab2/SFig26A_timing_fit3.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppTab2/SFig26A_timing_fit3.R")
...
outName <- paste0(outputDir, "/SFig26A_aID", aID, "_fit3_timing.txt.txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig26A_timing3_allZ")
betaName <- here::here(outputDir, "SFig26A_timing3_allBeta")
...
for (sim_it in 1:nSims) {
  ...
  if (doNew) {
    ...
    initPiList <- list(c(0.2/3, 0.2/3, 0.2/3), c(0.2/3, 0.2/3, 0.2/3), c(0.2/3, 0.2/3, 0.2/3), c(0.4/3, 0.4/3, 0.4/3))
    initMuList <- list(matrix(data=0, nrow=2, ncol=1), matrix(data=c(0, 1, 0, 3, 0, 5), nrow=2),
                     matrix(data=c(1, 0, 3, 0, 5, 0), nrow=2), matrix(data=c(7, 7, 8, 8, 9, 9), nrow=2)) 
