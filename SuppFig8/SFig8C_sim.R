...
# setwd("/path/to/csmGmm_reproduce/SuppFig8/SFig8C_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig8/SFig8C_sim.R")
...
outputDir <- here::here("SuppFig8", "output")
outName <- paste0(outputDir, "/sim_n1k_j100k_ind3d_changepi0_raiseAlt_randomeff_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- FALSE
testStatsName <- here::here(outputDir, "SFig8C_allZ")
betaName <- here::here(outputDir, "SFig8C_allBeta")
...
betaMin <- c(0.33, 0.33, 0.33)
betaMax <- c(0.43, 0.43, 0.43)
beta0 <- -1
