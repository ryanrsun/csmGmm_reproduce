...
# setwd("/path/to/csmGmm_reproduce/SuppFig20/SFig20B_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig20/SFig20B_sim.R")
...
outputDir <- here::here("SuppFig2", "output")
outName <- paste0(outputDir, "/SFig20B_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig20B_allZ")
betaName <- here::here(outputDir, "SFig20B_allBeta")
