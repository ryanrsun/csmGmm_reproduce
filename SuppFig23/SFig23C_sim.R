...
# setwd("/path/to/csmGmm_reproduce/SuppFig23/SFig23C_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig23/SFig23C_sim.R")
...
outName <- paste0(outputDir, "/SFig23C_aID", aID, ".txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig23C_allZ")
betaName <- here::here(outputDir, "SFig23C_allBeta")
...
betaOpts <- cbind(c(0.18 + 0.01 * effSizeMult, 0.28 + 0.01 * effSizeMult),
                  c(0.18 + 0.01 * effSizeMult, 0.28 + 0.01 * effSizeMult))
betaMin <- betaOpts[1, ]
betaMax <- betaOpts[2, ]
beta0 <- -1
...
for (sim_it in 1:nSims) {
  ...
  corRes <- symm_fit_cor_EM_noAssumption(testStats = as.matrix(allZ), corMat = estCorAll, initMuList = initMuListCor, initPiList = initPiListCor, eps=10^(-5))
