...
# setwd("/path/to/csmGmm_reproduce/SuppFig23/SFig23D_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig23/SFig23D_sim.R")
...
outName <- paste0(outputDir, "/SFig23D_aID", aID, ".txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig23D_allZ")
betaName <- here::here(outputDir, "SFig23D_allBeta")
...
betaOpts <- cbind(c(0.3 + 0.01 * effSizeMult, 0.4 + 0.01 * effSizeMult),
                  c(0.3 + 0.01 * effSizeMult, 0.4 + 0.01 * effSizeMult))
betaMin <- betaOpts[1, ]
betaMax <- betaOpts[2, ]
beta0 <- -1
...
for (sim_it in 1:nSims) {
  ...
  if (doNew) {
    ...
    newRes <- symm_fit_ind_EM_noAssumption(testStats = allZ, initMuList = initMuList, initPiList = initPiList, sameDirAlt=TRUE, eps=10^(-5))
    # record
