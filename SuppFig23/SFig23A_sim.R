...
# setwd("/path/to/csmGmm_reproduce/SuppFig23/SFig23A_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig23/SFig23A_sim.R")
...
outName <- paste0(outputDir, "/Fig23A_aID", aID, ".txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "Fig23A_allZ")
betaName <- here::here(outputDir, "Fig23A_allBeta")
...
betaMin <- c(0.24 + 0.01 * effSizeMult, 0.24 + 0.01 * effSizeMult, 0.24 + 0.01 * effSizeMult)
betaMax <- c(0.34 + 0.01 * effSizeMult, 0.34 + 0.01 * effSizeMult, 0.34 + 0.01 * effSizeMult)
beta0 <- -1
...
for (sim_it in 1:nSims) {
  ...
  if (doNew) {
    ...
    newRes <- symm_fit_ind_EM_noAssumption(testStats = allZ, initMuList = initMuList, initPiList = initPiList, eps=10^(-5))
