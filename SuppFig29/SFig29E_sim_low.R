...
# setwd("/path/to/csmGmm_reproduce/SuppFig29/SFig29E_sim_low.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig29/SFig29E_sim_low.R")
...
outName <- paste0(outputDir, "/SFig29E_aID", aID, "_fitlow.txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig29Efl_allZ")
betaName <- here::here(outputDir, "SFig29Efl_allBeta")
...
for (sim_it in 1:nSims) {
  ...
  if (doNew) {
    ...
    initMuList <- list(matrix(data=0, nrow=2, ncol=1), matrix(data=c(0, 3), nrow=2),
                     matrix(data=c(3, 0), nrow=2), matrix(data=c(4, 4), nrow=2)) 
