...
# setwd("/path/to/csmGmm_reproduce/SuppFig29/SFig29E_sim_high.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig29/SFig29E_sim_high.R")
...
outName <- paste0(outputDir, "/SFig29E_aID", aID, "_fithigh.txt")

# option to save or load intermediate data to save time
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig29Efh_allZ")
betaName <- here::here(outputDir, "SFig29Efh_allBeta")
...
for (sim_it in 1:nSims) {
  ...
  if (doNew) {
    ...
    initMuList <- list(matrix(data=0, nrow=2, ncol=1), matrix(data=c(0, 6), nrow=2),
                     matrix(data=c(6, 0), nrow=2), matrix(data=c(8, 8), nrow=2)) 
