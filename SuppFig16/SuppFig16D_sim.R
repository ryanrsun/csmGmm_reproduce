...
# setwd("/path/to/csmGmm_reproduce/SuppFig16/SuppFig16D_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig16/SuppFig16D_sim.R")
...
outName <- paste0(outputDir, "/SFig16D_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig16D_allZ")
betaName <- here::here(outputDir, "SFig16D_allBeta")
...
# make it asymmetric
hMat$number[c(2, 6, 7)] <- round(hMat$number[c(2, 6, 7)] * 0)
hMat$number[c(5, 8, 9)] <- round(hMat$number[c(5, 8, 9)] * 2)
