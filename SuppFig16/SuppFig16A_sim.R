...
# setwd("/path/to/csmGmm_reproduce/SuppFig16/SuppFig16A_sim.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
here::i_am("SuppFig16/SuppFig16A_sim.R")
...
outputDir <- here::here("SuppFig16", "output")
outName <- paste0(outputDir, "/SFig16A_aID", aID, ".txt")

# option to save or load intermediate data to save time,
# set as FALSE for first run and then TRUE thereafter
loadData <- FALSE
saveData <- TRUE
testStatsName <- here::here(outputDir, "SFig16A_allZ")
betaName <- here::here(outputDir, "SFig16A_allBeta")
...
# make it asymmetric
hMat$number[c(2, 6, 7)] <- round(hMat$number[c(2, 6, 7)] * 0.5)
hMat$number[c(5, 8, 9)] <- round(hMat$number[c(5, 8, 9)] * 1.5)
